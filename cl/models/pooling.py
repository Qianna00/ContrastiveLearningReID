...
class GeneralizedMeanPooling(nn.Module, ABC):
    ...
    def __init__(self, norm, output_size=1, eps=1e-6, extra=False):
    ...
    def forward(self, x):
        ...
        return torch.nn.functional.adaptive_avg_pool2d(x, 1).pow(1.0 / self.p), \
               torch.nn.functional.adaptive_avg_pool2d(x, self.output_size).pow(1.0 / self.p)
...
class GeneralizedMeanPoolingP(GeneralizedMeanPooling, ABC):
    ...
    def __init__(self, norm=3, output_size=(4, 9), eps=1e-6, extra=False):
        super(GeneralizedMeanPoolingP, self).__init__(norm, output_size, eps, extra)
...
class GeneralizedMeanPoolingFpn(nn.Module, ABC):
    ...
    def forward(self, x_lists):
        ...
        for x in x_lists:
            x = x.clamp(min=self.eps).pow(self.p)
            out = torch.nn.functional.adaptive_avg_pool2d(x, 1).pow(
                1.0 / self.p
            )
            outs.append(out)
