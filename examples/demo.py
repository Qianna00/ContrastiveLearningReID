...
from cl.models.dsbn import convert_dsbn, convert_bn
from cl.evaluators import Evaluator
from cl.utils.data import transforms as T
from cl.utils.data.preprocessor import Preprocessor
from cl.utils.logging import Logger
from cl.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict
from cl.evaluators import extract_features, pairwise_distance
...
def main_worker(args):
    ...
    selected_query = random.sample(query, 10)
    features, labels = extract_features(model, test_loader)
    for single_query in selected_query:
        print("query_image:", single_query)
        distmat, query_features, gallery_features = pairwise_distance(features, [single_query], gallery)
        # print(distmat.shape, query_features.shape, gallery_features.shape)
        indices = np.argsort(distmat.squeeze())
        top10_index = indices[:10]
        for i in range(10):
            ind = top10_index[i]
            ret = gallery[ind]
            print(ret)
